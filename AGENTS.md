...
- After every edit, run `bun format && bun lint && bunx tsc --noEmit && bun test` to ensure that the code passes static analysis. If the linter or type checker return any errors, or any tests fail, fix those problems before handing back control.
